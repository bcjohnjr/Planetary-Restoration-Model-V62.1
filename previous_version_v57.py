...
def _trajectory_rows() -> List[Dict[str, float]]:
    rows=[]
    with (HERE / "external_validation_net_co2_trajectory.csv").open(encoding="utf-8") as f:
...
def _fair_timeseries() -> List[Dict[str, float]]:
    rows=[]
    for r in _zip_csv_rows(HERE / "fair-v2.2.4-results.zip", "fair_v2_2_4_timeseries.csv"):
        rows.append({k: float(v) for k,v in r.items()})
    return rows


def _hector_timeseries() -> List[Dict[str, float]]:
    with zipfile.ZipFile(HERE / "hector-v3.5.0-results.zip") as z:
