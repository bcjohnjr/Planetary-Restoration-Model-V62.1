#!/usr/bin/env python3
"""Planetary Restoration Model Version 54 — lineage-recovery internal candidate.

This Version 54 source preserves the complete Version 53.1 computational core and restores historically dropped lineage modules. It integrates the core
...
Run: python planetary_restoration_model_v54.py
"""
from __future__ import annotations

MODEL_VERSION = "54.0"
RELEASE_LABEL = "Planetary Restoration Model Version 54"
...
# ============================================================================================
# VERSION 54 — LINEAGE RECOVERY LAYER
# Restores historically modeled modules that were lost from the V53.x release wrapper.
# The V53.1 Earth-system/climate/CDR core remains authoritative. Legacy energy modules are
# restored in RECONCILE_ONLY mode until the 86,181.327 TWh/yr external pre-CDR electricity
# screen is decomposed, preventing accidental double counting.
# ============================================================================================

V54_MODEL_VERSION = "54.0"
V54_RELEASE_LABEL = "Planetary Restoration Model Version 54 — lineage-recovery internal candidate"
V54_RELEASE_STATUS = "INTERNAL_LINEAGE_RECOVERY_CANDIDATE_EXTERNAL_FAIR_OSCAR_STILL_BLOCKING"


def _v54_crf(rate: float, years: int) -> float:
    return rate * (1.0 + rate) ** years / ((1.0 + rate) ** years - 1.0)


def seaweed_saf_v54() -> Dict:
    """Restore the v9/v18 macroalgae HTL aviation-fuel screen, preserving legacy scenarios."""
    cases = {
        "low": {"jet_density_kg_l":0.78,"dry_productivity_t_ha_yr":15.0,"fuel_yield_dry_mass":0.12,"mfsp_usd_gge":13.0,"process_energy_ratio":0.60},
        "central": {"jet_density_kg_l":0.80,"dry_productivity_t_ha_yr":33.3,"fuel_yield_dry_mass":0.17,"mfsp_usd_gge":9.0,"process_energy_ratio":0.40},
        "high": {"jet_density_kg_l":0.82,"dry_productivity_t_ha_yr":50.0,"fuel_yield_dry_mass":0.20,"mfsp_usd_gge":7.0,"process_energy_ratio":0.25},
    }
    gal_bn=104.0; liters=gal_bn*1e9*3.78541; fuel_energy_twh=3755.5555555556
    rows=[]
    for key,c in cases.items():
        mass_mt=liters*c["jet_density_kg_l"]/1e9
        dry_gt=mass_mt/1000.0/c["fuel_yield_dry_mass"]
        area_mkm2=dry_gt*1e9/c["dry_productivity_t_ha_yr"]/100.0/1e6
        process_twh=fuel_energy_twh*c["process_energy_ratio"]
        elec=process_twh*0.70
        rows.append({"case":key,"global_aviation_fuel_bn_us_gal_yr":gal_bn,"fuel_mass_mt_yr":mass_mt,
                     "dry_seaweed_gt_yr":dry_gt,"farm_area_million_km2":area_mkm2,
                     "external_process_energy_twh_eq_yr":process_twh,"electricity_twh_yr":elec,
                     "electricity_saved_vs_legacy_e_saf_twh_yr":12000.0-elec,
                     "mfsp_usd_gge":c["mfsp_usd_gge"],"annual_fuel_cost_usd_bn_yr":gal_bn*c["mfsp_usd_gge"],
                     "status":"RESTORED_LEGACY_SCREEN_NOT_ADDITIVE_TO_EXTERNAL_ELECTRICITY_BASE"})
    return {"rows":rows,"central":next(r for r in rows if r["case"]=="central"),
            "method":"Wet-biomass-compatible hydrothermal liquefaction/upgrading; not direct lipid extraction.",
            "double_count_rule":"Electricity is an explicit decomposition candidate within the V53.1 external pre-CDR electricity screen, not an added load until reconciliation."}


def grid_harvester_v54() -> Dict:
    """Restore the v19 physically reconstructed grid/drone seaweed harvester."""
    throughput=100.0; dry_frac=0.125; avail=0.75; D=0.21844; L=82.5; slurry_frac=0.30
    rho=1025.0; f=0.02; K=5.0; static=3.0; pump_eff=0.70; cutter_kwh_t=0.75
    controls_kw=50.0; knots=1.0; Cd=0.10; capex_m=7.5; life=15; r=0.07
    om_frac=0.05; crew_m=0.45; storm_frac=0.02; hose_m=0.15; electricity_usd_mwh=100.0
    wet_kg_s=throughput*1000/3600; slurry_kg_s=wet_kg_s/slurry_frac; q=slurry_kg_s/rho
    area=math.pi*D*D/4; v=q/area; g=9.80665
    head=f*(L/D)*v*v/(2*g)+K*v*v/(2*g)+static
    pump_kw=rho*g*q*head/pump_eff/1000
    cutter_kw=cutter_kwh_t*throughput
    u=knots*0.514444; drag_n=0.5*rho*u*u*Cd*D*L; drag_kw=drag_n*u/1000
    total_kw=pump_kw+cutter_kw+controls_kw+drag_kw
    hours=8760*avail; dry_t=throughput*dry_frac*hours; mwh=total_kw*hours/1000
    annual_cost_m=capex_m*_v54_crf(r,life)+capex_m*om_frac+crew_m+capex_m*storm_frac+hose_m+mwh*electricity_usd_mwh/1e6
    cost_dry=annual_cost_m*1e6/dry_t; reference_cost=52.25; saving=reference_cost-cost_dry
    dry_req_gt=seaweed_saf_v54()["central"]["dry_seaweed_gt_yr"]
    fleet=dry_req_gt*1e9/dry_t; global_twh=fleet*mwh/1e6
    gge_per_dry_t=104e9/(dry_req_gt*1e9); mfsp_saving=saving/gge_per_dry_t
    return {"central":{"wet_throughput_t_h":throughput,"dry_fraction":dry_frac,"availability":avail,
        "slurry_velocity_m_s":v,"total_pump_head_m":head,"pump_power_kw":pump_kw,"cutter_power_kw":cutter_kw,
        "hose_cross_current_force_n":drag_n,"total_operating_power_kw":133.74406270551114,
        "specific_electricity_kwh_wet_t":1.3374406270551114,"specific_electricity_kwh_dry_t":10.69952501644089,
        "annual_dry_harvest_t_unit":82125.0,"annual_electricity_mwh_unit":878.6984919752082,"annual_total_unit_cost_usd_m":2.0363295344550694,
        "harvest_cost_usd_dry_t":24.79548900401911,"saving_vs_legacy_reference_usd_dry_t":27.45451099598089,"mfsp_saving_usd_gge":0.4890662610321786,
        "required_fleet_units":22558.589808219178,"global_harvest_electricity_twh_yr":19.822198845569492,"fleet_capex_usd_bn":169.18942356164382,
        "v54_independent_recompute_total_power_kw":total_kw,"v54_independent_recompute_global_electricity_twh_yr":global_twh,
        "status":"RESTORED_V19_PHYSICAL_SCREEN"},
        "legacy_monte_carlo":{"energy_kwh_dry_t_p2p5":7.5118,"energy_kwh_dry_t_median":11.5497,"energy_kwh_dry_t_p97p5":17.2429,
                              "cost_usd_dry_t_p2p5":16.5676,"cost_usd_dry_t_median":26.4108,"cost_usd_dry_t_p97p5":42.3539,
                              "fleet_p2p5":14379.5,"fleet_median":21079.96,"fleet_p97p5":31272.37},
        "note":"The earlier 3.78x movement proxy is not treated as a validated efficiency claim; V54 restores the later v19 hydraulic/power/throughput reconstruction."}


def csp_htl_v54() -> Dict:
    """Restore the v18 direct concentrated-solar heat + HTL integration using v19 harvester economics."""
    base=seaweed_saf_v54()["central"]; h=grid_harvester_v54()["central"]
    # Reconstruct v19/v18 central economics while preserving the published lineage values.
    thermal_share=0.158/(0.158+0.0385+0.206); electric_share=1.0-thermal_share
    revised_process=1452.1481266666665
    heat_twh=revised_process*thermal_share; elec_twh=revised_process*electric_share
    dni=2500; heat_eff=.60; elec_eff=.20; land_mult=2.5
    heat_aperture=heat_twh*1e9/(dni*heat_eff)/1e6
    power_aperture=elec_twh*1e9/(dni*elec_eff)/1e6
    aperture=heat_aperture+power_aperture; land=aperture*land_mult
    return {"central":{"revised_external_process_energy_twh_eq_yr":revised_process,
        "direct_solar_process_heat_twhth_yr":heat_twh,"solar_electricity_and_h2_twh_yr":elec_twh,
        "electricity_saved_vs_v9_allocation_twh_yr":169.44325970186333,
        "heat_field_mirror_aperture_km2":heat_aperture,"power_field_mirror_aperture_km2":power_aperture,
        "total_mirror_aperture_km2":aperture,"indicative_csp_land_footprint_km2":land,
        "annual_process_cost_usd_bn_yr":58.3565,"annual_energy_saving_usd_bn_yr":18.2568,
        "v18_final_mfsp_usd_gge":8.399404045089767,"v19_final_mfsp_usd_gge":8.335387,
        "v19_annual_fuel_cost_usd_bn_yr":866.8803,"v19_annual_saving_vs_9_usd_gge_usd_bn_yr":69.1197,
        "grid_harvester_global_electricity_twh_yr":h["global_harvest_electricity_twh_yr"],
        "status":"RESTORED_LEGACY_SCREEN_NOT_ADDITIVE_TO_EXTERNAL_ELECTRICITY_BASE"},
        "caution":"Macroalgae HTL and solar integration remain screening pathways requiring pilot-scale validation and site-specific resource analysis."}


def seaweed_food_v54() -> Dict:
    rows=[]
    inputs={"central":(1.8526241882352943,.05,.10,2000,10.3,33.3),"low":(1.614098824,.02,.07,1500,8.2,50.0),"high":(2.55893716,.08,.15,2500,10.3,15.0)}
    for key,(fuel_gt,frac,protein_frac,kcal_kg,pop_b,yield_t) in inputs.items():
        total=fuel_gt/(1-frac); food_gt=total*frac; protein_mt=food_gt*1000*protein_frac
        gpd=protein_mt*1e12/(pop_b*1e9)/365
        kcalpd=food_gt*1e12*kcal_kg/(pop_b*1e9)/365
        area=total*1e9/yield_t/100/1e6
        rows.append({"case":key,"total_dry_seaweed_gt_yr":total,"food_grade_dry_seaweed_mt_yr":food_gt*1000,
                     "protein_mt_yr":protein_mt,"protein_g_person_day":gpd,"food_energy_kcal_person_day":kcalpd,
                     "share_reference_diet_energy":kcalpd/2355.0,"required_farm_area_million_km2":area,"share_world_ocean":area/361,
                     "status":"RESTORED_SCENARIO"})
    return {"rows":rows,"central":next(r for r in rows if r["case"]=="central"),
            "caution":"Direct human food must use appropriate edible species and processing; iodine and trace-element exposure prevent treating all biomass as unrestricted food."}


def wastewater_energy_v54(planning_population_b: float=10.3) -> Dict:
    cases={"low":{"pop_b":10.3,"ch4":20,"collection":.65,"capture":.65,"elec":.32,"parasitic":.35,"heat":.35,"uplift":.15},
           "central_legacy":{"pop_b":12.2,"ch4":28.71,"collection":.85,"capture":.80,"elec":.36,"parasitic":.25,"heat":.45,"uplift":.35},
           "high":{"pop_b":13.0,"ch4":35,"collection":.95,"capture":.90,"elec":.40,"parasitic":.15,"heat":.50,"uplift":.60}}
    rows=[]
    for key,c in cases.items():
        cap_bm3=c["pop_b"]*1e9*c["ch4"]*c["collection"]*c["capture"]/1e9
        energy=cap_bm3*9.97; gross=energy*c["elec"]; net=gross*(1-c["parasitic"]); useful=energy*c["heat"]
        rows.append({"case":key,"population_b":c["pop_b"],"captured_methane_billion_m3_yr":cap_bm3,"methane_energy_twhth_yr":energy,
                     "gross_electricity_twh_yr":gross,"net_electricity_twh_yr":net,"useful_heat_twhth_yr":useful,
                     "net_electricity_with_food_waste_twh_yr":net*(1+c["uplift"]),"status":"RESTORED_LEGACY_SCREEN"})
    # Current planning-population normalization, using the legacy central per-capita assumptions.
    c=cases["central_legacy"]; cap=planning_population_b*1e9*c["ch4"]*c["collection"]*c["capture"]/1e9; en=cap*9.97; gross=en*c["elec"]; net=gross*(1-c["parasitic"])
    current={"case":"central_normalized_to_current_planning_population","population_b":planning_population_b,"captured_methane_billion_m3_yr":cap,
             "methane_energy_twhth_yr":en,"gross_electricity_twh_yr":gross,"net_electricity_twh_yr":net,"useful_heat_twhth_yr":en*c["heat"],
             "net_electricity_with_food_waste_twh_yr":net*(1+c["uplift"]),"status":"V54_NORMALIZED_SCREEN"}
    return {"rows":rows+[current],"legacy_central":next(r for r in rows if r["case"]=="central_legacy"),"current_planning_central":current,
            "double_count_rule":"Generation is restored as an explicit decomposition candidate inside the existing external electricity screen; it is not subtracted from demand until that screen is decomposed."}


def nutrient_loops_v54() -> Dict:
    current=seaweed()
    legacy={"effective_n_fertilizer_replacement_fraction":.2565,"effective_p_fertilizer_replacement_fraction":.0825,"effective_k_fertilizer_replacement_fraction":.2775,
            "legacy_gross_coastal_n_capture_mt_yr":8.835415,"legacy_gross_coastal_p_capture_mt_yr":1.1134,
            "nutrient_loop_food_multiplier":1.025,"coastal_seaweed_resilience_multiplier":1.01}
    legacy.update({"v53_1_credited_n_mt_yr":current["credited_eutrophic_n_mt_yr"],"v53_1_credited_p_mt_yr":current["credited_eutrophic_p_mt_yr"],
                   "n_crediting_ratio_vs_legacy_gross":current["credited_eutrophic_n_mt_yr"]/8.835415,
                   "p_crediting_ratio_vs_legacy_gross":current["credited_eutrophic_p_mt_yr"]/1.1134,
                   "status":"RESTORED_WITH_UNDOCUMENTED_LINEAGE_HAIRCUT_EXPOSED"})
    return {"central":legacy,"interpretation":"V54 retains both the older gross capture screen and the V53.1 credited values rather than silently choosing one; the approximately 50% crediting difference remains an explicit reconciliation item."}


def tidal_energy_v54() -> Dict:
    cases={"conservative":(600,300,.12,15,180,.70,208),"central":(1200,800,.10,10,140,.80,866),"high":(1800,1500,.08,8,110,.90,2110)}
    rows=[]
    for key,(stream,range_,loss,gco2,lcoe,firm,waste) in cases.items():
        gross=stream+range_; net=gross*(1-loss); firm_twh=net*firm
        rows.append({"case":key,"gross_tidal_twh_yr":gross,"net_delivered_tidal_twh_yr":net,"firm_value_equivalent_twh_yr":firm_twh,
                     "lifecycle_emissions_mtco2e_yr":net*gco2/1000,"annual_generation_cost_usd_bn_yr":net*lcoe/1000,
                     "legacy_clean_energy_gap_after_wastewater_tidal_twh_yr":12581.33-waste-net,
                     "status":"RESTORED_RECONCILE_ONLY"})
    return {"rows":rows,"central":next(r for r in rows if r["case"]=="central"),
            "double_count_rule":"Not added to the V53.1 external electricity base until the base is decomposed by technology."}


def global_maritime_v54() -> Dict:
    rows=[
        {"metric":"Baseline propulsion-equivalent demand","value":9000.0,"unit":"TWh/yr"},
        {"metric":"After efficiency/logistics","value":7200.0,"unit":"TWh/yr"},
        {"metric":"After wind assist","value":6120.0,"unit":"TWh/yr"},
        {"metric":"Battery-electric propulsion service","value":918.0,"unit":"TWh/yr"},
        {"metric":"Hydrogen-derived propulsion service","value":2754.0,"unit":"TWh/yr"},
        {"metric":"Synthetic-fuel propulsion service","value":1836.0,"unit":"TWh/yr"},
        {"metric":"Biofuel propulsion service","value":612.0,"unit":"TWh/yr"},
        {"metric":"Battery electricity","value":1080.0,"unit":"TWh/yr"},
        {"metric":"Hydrogen-derived fuel electricity","value":7868.571428571429,"unit":"TWh/yr"},
        {"metric":"Synthetic-fuel electricity","value":6120.0,"unit":"TWh/yr"},
        {"metric":"Biofuel processing electricity","value":61.2,"unit":"TWh/yr"},
        {"metric":"Total shipping electricity","value":15129.77142857143,"unit":"TWh/yr"},
        {"metric":"Legacy baseline shipping GHG","value":1.0,"unit":"GtCO2e/yr"},
    ]
    return {"rows":rows,"central_total_shipping_electricity_twh_yr":15129.77142857143,
            "status":"RESTORED_DETAILED_LOAD_DECOMPOSITION_RECONCILE_ONLY",
            "double_count_rule":"Do not add this load to the V53.1 external pre-CDR electricity screen until that screen is decomposed."}


def storage_portfolio_v54() -> Dict:
    rows=[
        {"technology":"Li-ion / sodium batteries","power_gw":4500,"duration_h":4,"energy_gwh":18000,"roundtrip_efficiency":.85,"cycles_yr":250,"annual_discharged_twh":3825,"firm_credit":.90},
        {"technology":"Long-duration storage","power_gw":2500,"duration_h":24,"energy_gwh":60000,"roundtrip_efficiency":.65,"cycles_yr":80,"annual_discharged_twh":3120,"firm_credit":.80},
        {"technology":"Pumped hydro","power_gw":2200,"duration_h":12,"energy_gwh":26400,"roundtrip_efficiency":.80,"cycles_yr":120,"annual_discharged_twh":2534.4,"firm_credit":.90},
        {"technology":"Hydrogen / e-fuel power-to-power","power_gw":1500,"duration_h":120,"energy_gwh":180000,"roundtrip_efficiency":.35,"cycles_yr":20,"annual_discharged_twh":1260,"firm_credit":.95},
        {"technology":"Thermal / district heat storage","power_gw":1800,"duration_h":10,"energy_gwh":18000,"roundtrip_efficiency":.90,"cycles_yr":150,"annual_discharged_twh":2430,"firm_credit":.75},
        {"technology":"Smart EV / V2G","power_gw":3000,"duration_h":3,"energy_gwh":9000,"roundtrip_efficiency":.88,"cycles_yr":120,"annual_discharged_twh":950.4,"firm_credit":.55},
    ]
    return {"rows":rows,"totals":{"power_gw":15500,"energy_gwh":311400,"annual_discharged_twh":14119.8,"weighted_firm_gw":12455},
            "status":"RESTORED_TECHNOLOGY_PORTFOLIO; CURRENT_V53_SYNTHETIC_8760_ADEQUACY_REMAINS_AUTHORITATIVE"}


def firm_clean_options_v54() -> Dict:
    raw=[("New nuclear",0,.90,12,6500,1.0,0),("Enhanced geothermal",500,.90,20,5000,1.0,2500000),
         ("Hydro / pumped-turbine expansion",400,.45,24,3500,.90,1400000),("H2 / biomethane turbines",500,.12,25,1200,.90,600000),
         ("Long-duration firming expansion",800,.10,25,2500,.80,2000000)]
    rows=[]
    for tech,gw,cf,life_g,capex_kw,firm,legacy_capex in raw:
        twh=gw*8.76*cf; formula_bn=gw*1e6*capex_kw/1e9
        rows.append({"technology":tech,"capacity_gw":gw,"capacity_factor":cf,"generation_twh_yr":twh,"lifecycle_gco2_kwh":life_g,
                     "capex_usd_kw":capex_kw,"firm_credit":firm,"legacy_capex_cell_value_labeled_usd_bn":legacy_capex,
                     "formula_reconciled_capex_usd_bn":formula_bn,"unit_reconciliation_status":"LEGACY_CAPEX_LABEL_APPEARS_1000X_HIGH" if legacy_capex and abs(legacy_capex/formula_bn-1000)<1 else "OK",
                     "status":"RESTORED_OPTION_SCREEN"})
    return {"rows":rows,"interpretation":"The historical zero-new-nuclear central conclusion is superseded by V53.1 synthetic adequacy results; V54 restores technology options, not the old adequacy conclusion."}


def shock_resilience_v54() -> Dict:
    rows=[
        {"shock":"Major multi-region drought","annual_probability":.04,"duration_years":2,"food_multiplier":.90,"energy_multiplier":.98,"gdp_multiplier":.98,"infrastructure_cost_multiplier":1.05,"recovery_reserve_fraction":.15,"legacy_threshold":">=90% welfare coverage","legacy_status":"PASS"},
        {"shock":"Extreme renewable drought / dunkelflaute","annual_probability":.03,"duration_years":.05,"food_multiplier":1,"energy_multiplier":.90,"gdp_multiplier":.995,"infrastructure_cost_multiplier":1.03,"recovery_reserve_fraction":.20,"legacy_threshold":">=99.9% critical-load supply","legacy_status":"PASS"},
        {"shock":"Major trade fragmentation","annual_probability":.03,"duration_years":5,"food_multiplier":.95,"energy_multiplier":.96,"gdp_multiplier":.94,"infrastructure_cost_multiplier":1.10,"recovery_reserve_fraction":.20,"legacy_threshold":">=90% essentials regionalized","legacy_status":"PASS"},
        {"shock":"Pandemic-scale health shock","annual_probability":.02,"duration_years":2,"food_multiplier":.98,"energy_multiplier":.99,"gdp_multiplier":.95,"infrastructure_cost_multiplier":1.08,"recovery_reserve_fraction":.20,"legacy_threshold":"health surge reserve","legacy_status":"PASS"},
        {"shock":"Technology deployment delay","annual_probability":.10,"duration_years":5,"food_multiplier":.97,"energy_multiplier":.94,"gdp_multiplier":.97,"infrastructure_cost_multiplier":1.15,"recovery_reserve_fraction":.25,"legacy_threshold":"carbon target slips rather than welfare failure","legacy_status":"PASS"},
        {"shock":"Severe wildfire / forest reversal year","annual_probability":.05,"duration_years":1,"food_multiplier":.99,"energy_multiplier":.99,"gdp_multiplier":.995,"infrastructure_cost_multiplier":1.03,"recovery_reserve_fraction":.15,"legacy_threshold":"durable CDR portfolio diversified","legacy_status":"PASS"},
    ]
    for r in rows:r["v54_status"]="RESTORED_STRESS_PARAMETER; LEGACY_PASS_NOT_REVALIDATED"
    return {"rows":rows,"caution":"V54 restores the shock mechanisms but does not carry forward legacy PASS labels as new validation results."}


def tree_wealth_v54() -> Dict:
    rows=[
        {"mechanism":"National Natural-Capital Reserve","formula":"audited timber NPV + verified durable carbon value x permanence haircut","coin_supply_effect":"No automatic money creation","safeguard":"Independent audit + fire/drought buffer","model_treatment":"RESTORED_POLICY"},
        {"mechanism":"Planting-contract revenue","formula":"climate fund pays verified additional planting/restoration removal","coin_supply_effect":"No supply change","safeguard":"additionality + survival monitoring","model_treatment":"RESTORED_POLICY"},
        {"mechanism":"Growth dividend","formula":"optional issuance only after measured biomass gain","coin_supply_effect":"capped and sterilized by fee-burn/global supply rule","safeguard":"global inflation ceiling","model_treatment":"RESTORED_OPTION"},
        {"mechanism":"Harvest-to-durable-storage rule","formula":"harvest only if right retired or carbon routed to long-lived product/wood vault","coin_supply_effect":"supply neutral or coin burn","safeguard":"chain of custody","model_treatment":"RESTORED_POLICY"},
        {"mechanism":"Permanence pool","formula":"10–20% unassigned forest/carbon buffer","coin_supply_effect":"none","safeguard":"geographic diversification + MRV","model_treatment":"RESTORED_REQUIRED_BUFFER"},
    ]
    return {"rows":rows,"default_permanence_pool_range_fraction":[.10,.20],"status":"RESTORED"}


def country_equalizer_v54() -> Dict:
    weights={"absolute_verified_net_removal":.40,"mobilization_of_sustainable_domestic_potential":.30,"climate_finance_relative_to_gdp":.15,"ecological_quality":.15}
    examples=[
        {"archetype":"Saudi-type arid/land constrained","eligible_land_index":.1,"growth_index":.3,"permanence_index":.85,"ecology_water_safeguard":.8,"adjusted_tree_removal_index":.0204,"complement":"solar DAC + mineral carbonation + storage/finance"},
        {"archetype":"Canada/Russia-type boreal","eligible_land_index":1.5,"growth_index":.6,"permanence_index":.75,"ecology_water_safeguard":.9,"adjusted_tree_removal_index":.6075,"complement":"boreal restoration + wood/BiCRS + DAC/mineralization"},
        {"archetype":"Brazil-type tropical","eligible_land_index":1.0,"growth_index":1.6,"permanence_index":.65,"ecology_water_safeguard":.85,"adjusted_tree_removal_index":.884,"complement":"native restoration + biochar + avoided degradation"},
    ]
    return {"weights":weights,"examples":examples,"central_equalizer_access_pool_usd_bn_yr":80.74427824833302,
            "principles":["Same permanence-adjusted price per verified net tonne","No national tree quota","No nation required to exceed audited sustainable domestic potential","Forestry never mandatory; engineered CDR/finance can fulfill contribution"],"status":"RESTORED"}


def national_allocation_v54() -> Dict:
    rows=[
        {"rule":"M_i^USD = BroadMoney_i / FX_i","purpose":"measure existing monetary stock in common value"},
        {"rule":"Share_i = M_i^USD / sum(M_j^USD)","purpose":"preserve relative monetary wealth"},
        {"rule":"Coins_i = TotalGenesis x Share_i","purpose":"proportional conversion to existing holders"},
        {"rule":"Government reserve tranche converted separately","purpose":"preserve sovereign reserve assets"},
        {"rule":"90-day pre-announced FX averaging","purpose":"limit one-day manipulation"},
        {"rule":"Immediate national wealth impact approximately zero by construction","purpose":"avoid arbitrary transfer between nations"},
    ]
    return {"rows":rows,"example":national_balance({"Country A":50,"Country B":30,"Country C":20}),
            "legacy_12bp_fee_burden_fraction_mature_gdp":.0216,"status":"RESTORED_POLICY_ARCHITECTURE"}


def parallel_settlement_v54() -> Dict:
    rows=[
        {"system":"Treaty-governed global settlement currency","can_coexist":"YES","un_core_control":"Treaty-governed","national_regulation":"YES","sanctions":"UN-only sanctions at core in primary design","climate_finance_leakage":"None on participating flows","legacy_share":"95% central participation"},
        {"system":"Bitcoin / permissionless crypto","can_coexist":"YES","un_core_control":"NO","national_regulation":"Jurisdiction-dependent","sanctions":"UN protocol itself cannot freeze permissionless protocol; regulated intermediaries remain subject to law","climate_finance_leakage":"Some flows can bypass climate fees","legacy_share":"part of 5% parallel-flow sensitivity"},
        {"system":"Dollar/euro/local-currency side settlement","can_coexist":"YES during transition","un_core_control":"NO","national_regulation":"YES","sanctions":"state legal systems apply","climate_finance_leakage":"reduces fee base","legacy_share":"transition sensitivity"},
    ]
    return {"rows":rows,"status":"RESTORED_GOVERNANCE_SENSITIVITY"}


def sanctions_governance_v54() -> Dict:
    rows=[
        {"mode":"UN-only sanctions at core","treatment":"PRIMARY TREATY DESIGN","implication":"narrow Security Council Article 41 path; no unilateral private/single-country freeze at core"},
        {"mode":"Protocol-neutral core","treatment":"TECHNICALLY POSSIBLE / LEGALLY COMPLEX","implication":"member states may still be obliged to implement mandatory UN sanctions outside protocol core"},
        {"mode":"National-sanctions mode","treatment":"NOT RECOMMENDED","implication":"fragmentation and geopolitical weaponization risk"},
    ]
    return {"rows":rows,"status":"RESTORED"}


def dollar_transition_v54() -> Dict:
    rows=[
        {"metric":"US dollar share of disclosed FX reserves, 2026Q1","value":57.13,"unit":"percent","status":"OBSERVED_LEGACY_SOURCE","treatment":"Explicit constraint; de-dollarization not assumed inevitable"},
        {"metric":"US dollar reserve share, 2001 peak","value":72.0,"unit":"percent","status":"HISTORICAL_BENCHMARK","treatment":"Long-run diversification context"},
        {"metric":"US international-currency usage index, 2024","value":64.9,"unit":"index percent","status":"OBSERVED_LEGACY_SOURCE","treatment":"Transition must be coordinated rather than assumed"},
    ]
    return {"rows":rows,"status":"RESTORED_TRANSITION_CONTEXT"}


def no_new_burden_v54() -> Dict:
    old_burden=2.5/110.98
    return {"rows":[
        {"metric":"Current global payments revenue","value":2.5,"unit":"USD tn/yr"},
        {"metric":"Current world GDP reference","value":110.98,"unit":"USD tn/yr"},
        {"metric":"Legacy observed payments-revenue burden","value":old_burden,"unit":"fraction GDP"},
        {"metric":"Current V53.1 settlement envelope","value":FinanceInputs().settlement_envelope_fraction_gdp,"unit":"fraction GDP"},
        {"metric":"Protected basic food payments","value":"fee-free","unit":"policy"},
        {"metric":"Protected health payments","value":"fee-free / no climate surcharge","unit":"policy"},
    ],"status":"RESTORED_WITH_CURRENT_CAP_SHOWN_SIDE_BY_SIDE"}


def food_health_budget_v54() -> Dict:
    pop=10.3; food=1800; health=2500
    rows=[
        {"budget_item":"Healthy-food allowance","usd_person_yr":food,"planning_population_b":pop,"annual_resource_envelope_usd_tn":food*pop/1000,"share_mature_gdp":.06,"fee_treatment":"basic-food payments fee-free"},
        {"budget_item":"Universal basic health-care allowance","usd_person_yr":health,"planning_population_b":pop,"annual_resource_envelope_usd_tn":health*pop/1000,"share_mature_gdp":1/12,"fee_treatment":"health payments fee-free; no climate surcharge"},
        {"budget_item":"Combined food + health","usd_person_yr":None,"planning_population_b":pop,"annual_resource_envelope_usd_tn":44.29,"share_mature_gdp":.14333333333333334,"fee_treatment":"inside decent-living economy, not added to climate transaction fee"},
    ]
    return {"rows":rows,"status":"RESTORED_RESOURCE_BUDGET"}


def ai_data_center_transition_v54() -> Dict:
    market=[
        {"metric":"Global data-center capacity demand","current_2025":82,"2030":219,"growth_or_share":.217099,"unit":"GW / CAGR","status":"RESTORED_LEGACY_MARKET_ANCHOR"},
        {"metric":"AI data-center capacity demand","current_2025":44,"2030":156,"growth_or_share":.288055,"unit":"GW / CAGR","status":"RESTORED_LEGACY_MARKET_ANCHOR"},
        {"metric":"Cumulative data-center capex through 2030","current_2025":None,"2030":6.7,"growth_or_share":None,"unit":"USD tn","status":"RESTORED_LEGACY_MARKET_ANCHOR"},
        {"metric":"AI-specific cumulative capex through 2030","current_2025":None,"2030":5.2,"growth_or_share":5.2/6.7,"unit":"USD tn / share","status":"RESTORED_LEGACY_MARKET_ANCHOR"},
        {"metric":"Global data-center electricity demand","current_2025":485,"2030":950,"growth_or_share":.143922,"unit":"TWh/yr / CAGR","status":"RESTORED_LEGACY_MARKET_ANCHOR"},
    ]
    distributed=[
        {"metric":"Baseline Year-20 network sites","value":20000,"unit":"sites","status":"INTERNAL_MODEL"},
        {"metric":"Standard GPU configuration","value":35,"unit":"B300 GPUs/site","status":"INTERNAL_MODEL"},
        {"metric":"Parent Year-20 EBITDA","value":5.62,"unit":"USD bn/yr","status":"INTERNAL_MODEL"},
    ]
    cella=[
        {"metric":"Municipalities in project scope","value":8,"unit":"municipalities","status":"DESIGN_MODEL"},
        {"metric":"Distributed data centers","value":6,"unit":"facilities","status":"DESIGN_MODEL"},
        {"metric":"Continuous IT load","value":35.74,"unit":"MW","status":"DESIGN_MODEL"},
        {"metric":"Reference PUE","value":1.45,"unit":"PUE","status":"DESIGN_MODEL"},
        {"metric":"Project PUE","value":1.223,"unit":"PUE","status":"DESIGN_MODEL"},
        {"metric":"Recoverable data-center heat","value":275.6,"unit":"GWh/yr","status":"DESIGN_MODEL"},
        {"metric":"Useful recovered heat","value":239.8,"unit":"GWh/yr","status":"DESIGN_MODEL"},
        {"metric":"District heat delivered","value":51.4,"unit":"GWh/yr","status":"DESIGN_MODEL"},
    ]
    return {"market_rows":market,"distributed_network_rows":distributed,"cella_rows":cella,
            "current_bank_screen":banking_transition(),"status":"RESTORED_IMPLEMENTATION_LAYER"}


def workforce_transition_v54() -> Dict:
    affected=10.0
    allocations=[("CDR/clean-energy project finance",.16),("Treasury/trade/SME credit",.14),("Public payment utility",.10),("Cybersecurity/fraud/identity/privacy",.10),
                 ("Climate MRV",.10),("Insurance/catastrophe risk",.10),("AI/robotics finance/leasing/operations",.10),("Forestry/natural capital",.08),("Infrastructure/housing/water",.08),("Entrepreneurship",.04)]
    rows=[{"target_role":name,"share":share,"affected_workers_million":affected*share} for name,share in allocations]
    return {"rows":rows,"central_affected_financial_workers_million":affected,"annual_transition_need_usd_bn_yr":130.0,"existing_transition_fund_usd_bn_yr":334.1,"status":"RESTORED_PLANNING_STRESS_NOT_FORECAST"}


def cross_border_fee_v54() -> Dict:
    return {"current_cross_border_revenue_margin_bp":15.26315789473684,"mature_cross_border_flows_usd_tn_yr":529.0142367994233,
            "additional_cross_border_climate_revenue_usd_tn_yr":.8074427824833302,"equalizer_access_pool_usd_bn_yr":80.74427824833302,
            "direct_additional_cdr_fund_usd_tn_yr":.7266985042349973,"legacy_incremental_effective_cdr_gtco2_yr":3.3031750192499874,
            "legacy_combined_fee_burden_fraction_mature_gdp":.024238601549828796,
            "old_restoration_dates":"SUPERSEDED_BY_V53_1_EARTH_SYSTEM_CORE","status":"RESTORED_FINANCE_MECHANISM; OLD_CO2_DATES_NOT_RESTORED"}


def restored_modules_v54() -> Dict:
    return {
        "seaweed_saf_htl":seaweed_saf_v54(),"grid_harvester":grid_harvester_v54(),"csp_htl":csp_htl_v54(),"seaweed_food":seaweed_food_v54(),
        "wastewater_food_waste_bioenergy":wastewater_energy_v54(),"nutrient_loops_npk":nutrient_loops_v54(),"tidal_energy":tidal_energy_v54(),
        "global_maritime":global_maritime_v54(),"storage_portfolio":storage_portfolio_v54(),"firm_clean_options":firm_clean_options_v54(),
        "shock_resilience":shock_resilience_v54(),"tree_wealth":tree_wealth_v54(),"national_allocation":national_allocation_v54(),
        "country_equalizer":country_equalizer_v54(),"parallel_settlement":parallel_settlement_v54(),"sanctions_governance":sanctions_governance_v54(),
        "dollar_transition":dollar_transition_v54(),"no_new_burden":no_new_burden_v54(),"food_health_budget":food_health_budget_v54(),
        "ai_data_center_transition":ai_data_center_transition_v54(),"workforce_transition":workforce_transition_v54(),"cross_border_fee":cross_border_fee_v54(),
    }


def lineage_register_v54() -> List[Dict]:
    rows=[
        ("Seaweed SAF / HTL","v9/v18/v19","RESTORED","RECONCILE_ONLY electricity"),("Grid seaweed harvester","v18/v19","RESTORED","Physical v19 screen; no raw 3.78x claim"),
        ("Seaweed human food","v11","RESTORED","Scenario only"),("Wastewater + food-waste bioenergy","v13","RESTORED","RECONCILE_ONLY electricity"),
        ("N/P/K nutrient loops","v13","RESTORED","Gross legacy capture + V53.1 credited haircut shown together"),("Tidal power","v14","RESTORED","RECONCILE_ONLY supply"),
        ("Storage technology portfolio","v15","RESTORED","Current synthetic 8760 adequacy remains authoritative"),("Firm-clean options","v15","RESTORED","Old zero-nuclear conclusion superseded"),
        ("Shock resilience suite","v16","RESTORED","Legacy PASS not revalidated"),("Tree natural-capital architecture","v20","RESTORED","Policy/valuation layer"),
        ("National genesis allocation","v20","RESTORED","Policy architecture"),("Country contribution equalizer","v21","RESTORED","Technology-neutral fairness layer"),
        ("Parallel settlement / Bitcoin leakage","v22","RESTORED","Governance sensitivity"),("Sanctions governance","v21","RESTORED","Governance sensitivity"),
        ("Dollar transition context","v22","RESTORED","Context; not deterministic de-dollarization"),("No-new-burden + protected transactions","v22","RESTORED","Current settlement cap shown beside legacy benchmark"),
        ("Food + universal basic health budget","v22","RESTORED","Resource screen"),("Distributed AI/data-center banking transition","v23","RESTORED","Implementation/economic screen"),
        ("Los Jardines Cella demonstration archetype","v23","RESTORED","Design model, not operational deployment"),("Workforce transition","v21","RESTORED","Planning stress, not forecast"),
        ("Detailed maritime decarbonization","v8/v9","RESTORED","RECONCILE_ONLY load"),("Cross-border fee/equalizer funding","v21","RESTORED","Old restoration dates superseded"),
        ("Current water/seaweed/nutrients/plastics core screens","V53.1 source","RE-EXPOSED","Were present in source but missing from release wrapper"),
    ]
    return [{"module":a,"source_lineage":b,"v54_status":c,"coupling_rule":d} for a,b,c,d in rows]


def energy_reconciliation_v54(restored: Dict) -> Dict:
    external=BaseSystemInputs().pre_cdr_transformed_electricity_twh_yr
    return {"mode":"RECONCILE_ONLY","external_pre_cdr_electricity_twh_yr":external,
            "legacy_explicit_load_candidates_twh_yr":{"shipping":restored["global_maritime"]["central_total_shipping_electricity_twh_yr"],
                                                      "seaweed_saf_process":restored["seaweed_saf_htl"]["central"]["electricity_twh_yr"],
                                                      "grid_harvester":restored["grid_harvester"]["central"]["global_harvest_electricity_twh_yr"]},
            "legacy_explicit_supply_candidates_twh_yr":{"wastewater_plus_food_waste":restored["wastewater_food_waste_bioenergy"]["legacy_central"]["net_electricity_with_food_waste_twh_yr"],
                                                        "tidal_net":restored["tidal_energy"]["central"]["net_delivered_tidal_twh_yr"]},
            "headline_adjustment_twh_yr":0.0,
            "rule":"No restored legacy load or supply is added to/subtracted from the 86,181.327 TWh/yr external screen until that screen is decomposed; this prevents double counting."}


def run_v54_release(ensemble_samples: int=700) -> Dict:
    result=run_v53_release(ensemble_samples)
    restored=restored_modules_v54()
    current={"water":water(),"seaweed":seaweed(),"nutrients":nutrients(),"plastics":plastics(),
             "banking_transition":banking_transition(),"external_social_ecological_screens":external_social_ecological_screens()}
    result.update({"model_version":V54_MODEL_VERSION,"release_label":V54_RELEASE_LABEL,"release_status":V54_RELEASE_STATUS,
                   "classification":"V53.1 Earth-system validation candidate plus Version 54 lineage-recovery layer. Restored legacy modules remain screening/reconciliation layers unless explicitly coupled; precise climate restoration dates remain externally unvalidated.",
                   "current_resource_screens":current,"restored_modules":restored,"lineage_register":lineage_register_v54(),
                   "energy_reconciliation":energy_reconciliation_v54(restored)})
    h=result["headline"]
    h.update({"v54_restored_module_count":len(restored),"v54_lineage_register_count":len(result["lineage_register"]),
              "v54_energy_reconciliation_headline_adjustment_twh_yr":0.0,
              "v54_seaweed_food_kcal_person_day":restored["seaweed_food"]["central"]["food_energy_kcal_person_day"],
              "v54_wastewater_foodwaste_electricity_twh_yr_legacy_central":restored["wastewater_food_waste_bioenergy"]["legacy_central"]["net_electricity_with_food_waste_twh_yr"],
              "v54_tidal_net_twh_yr_central":restored["tidal_energy"]["central"]["net_delivered_tidal_twh_yr"],
              "v54_grid_harvester_twh_yr":restored["grid_harvester"]["central"]["global_harvest_electricity_twh_yr"]})
    return result


def _v54_rows_from_dict(d: Mapping, prefix: str="") -> List[Dict]:
    rows=[]
    for k,v in d.items():
        key=f"{prefix}.{k}" if prefix else str(k)
        if isinstance(v,dict): rows.extend(_v54_rows_from_dict(v,key))
        elif isinstance(v,list): rows.append({"metric":key,"value":json.dumps(v,ensure_ascii=False)})
        else: rows.append({"metric":key,"value":v})
    return rows


def write_v54_outputs(result: Dict, root: Path) -> None:
    data=root/"data";data.mkdir(exist_ok=True)
    compact={k:v for k,v in result.items() if k not in ("primary_v52_state","central_joos_diagnostic","full_advanced_sensitivity_ensemble")}
    compact["headline"]=result["headline"]
    compact["full_advanced_sensitivity_ensemble_summary"]={k:v for k,v in result["full_advanced_sensitivity_ensemble"].items() if k not in ("draw_rows","return_year_samples")}
    (data/"planetary_restoration_v54_results.json").write_text(json.dumps(compact,indent=2,ensure_ascii=False),encoding="utf-8")
    _write_csv_v53(data/"v54_lineage_register.csv",result["lineage_register"])
    _write_csv_v53(data/"v54_energy_reconciliation.csv",_v54_rows_from_dict(result["energy_reconciliation"]))
    r=result["restored_modules"]
    exports={
        "v54_seaweed_saf.csv":r["seaweed_saf_htl"]["rows"],"v54_grid_harvester.csv":_v54_rows_from_dict(r["grid_harvester"]["central"]),
        "v54_csp_htl.csv":_v54_rows_from_dict(r["csp_htl"]["central"]),"v54_seaweed_food.csv":r["seaweed_food"]["rows"],
        "v54_wastewater_energy.csv":r["wastewater_food_waste_bioenergy"]["rows"],"v54_nutrient_loops.csv":_v54_rows_from_dict(r["nutrient_loops_npk"]["central"]),
        "v54_tidal_energy.csv":r["tidal_energy"]["rows"],"v54_global_maritime.csv":r["global_maritime"]["rows"],
        "v54_storage_portfolio.csv":r["storage_portfolio"]["rows"],"v54_firm_clean_options.csv":r["firm_clean_options"]["rows"],
        "v54_shock_resilience.csv":r["shock_resilience"]["rows"],"v54_tree_wealth.csv":r["tree_wealth"]["rows"],
        "v54_national_allocation.csv":r["national_allocation"]["rows"],"v54_country_equalizer_examples.csv":r["country_equalizer"]["examples"],
        "v54_parallel_settlement.csv":r["parallel_settlement"]["rows"],"v54_sanctions_governance.csv":r["sanctions_governance"]["rows"],
        "v54_dollar_transition.csv":r["dollar_transition"]["rows"],"v54_no_new_burden.csv":r["no_new_burden"]["rows"],
        "v54_food_health_budget.csv":r["food_health_budget"]["rows"],"v54_ai_data_center_market.csv":r["ai_data_center_transition"]["market_rows"],
        "v54_distributed_ai_network.csv":r["ai_data_center_transition"]["distributed_network_rows"],"v54_cella_archetype.csv":r["ai_data_center_transition"]["cella_rows"],
        "v54_workforce_transition.csv":r["workforce_transition"]["rows"],"v54_cross_border_fee.csv":_v54_rows_from_dict(r["cross_border_fee"]),
    }
    for name,rows in exports.items(): _write_csv_v53(data/name,rows)
    # Preserve the core annual state and advanced ensemble draws for a self-contained release.
    _write_csv_v53(data/"annual_state.csv",result["primary_v52_state"]["annual_state"])
    _write_csv_v53(data/"v54_full_advanced_ensemble_draws.csv",result["full_advanced_sensitivity_ensemble"].get("draw_rows",[]))
    md=["# Version 54 lineage recovery","",
        "Version 54 restores historically modeled modules that disappeared from the V53.x release wrapper without an explicit scientific withdrawal.","",
        "## Coupling rule","",
        "The V53.1 Earth-system/climate/CDR core remains authoritative. Restored electricity loads and supplies are **not** added to the 86,181.327 TWh/yr undecomposed pre-CDR electricity screen until that screen is decomposed. This avoids double counting.","",
        "## Restored modules","",
    ]
    for x in result["lineage_register"]: md.append(f"- **{x['module']}** — {x['v54_status']} ({x['source_lineage']}); {x['coupling_rule']}.")
    md += ["","## Climate headline","",f"The physical headline is unchanged by the lineage-recovery wrapper: the full-advanced ensemble conditional median remains {result['headline']['headline_return_year_conditional_full_advanced_ensemble_median']} for this run. Precise restoration dates remain blocked on external calibrated/constrained FaIR and archived OSCAR validation."]
    (root/"LINEAGE_RECOVERY_V54.md").write_text("\n".join(md),encoding="utf-8")


def _v54_write_sheet(wb, name: str, rows: Sequence[Mapping], header_color: str="#1F4E78"):
    sh=wb.worksheets.add(name)
    if not rows:
        sh.get_range("A1").values=[["No rows"]];return sh
    cols=[]
    for row in rows:
        for k in row.keys():
            if k not in cols: cols.append(k)
    matrix=[cols]+[[json.dumps(row.get(c),ensure_ascii=False) if isinstance(row.get(c),(dict,list)) else row.get(c) for c in cols] for row in rows]
    sh.get_range_by_indexes(0,0,len(matrix),len(cols)).values=matrix
    sh.get_range_by_indexes(0,0,1,len(cols)).format={"fill":header_color,"font":{"bold":True,"color":"#FFFFFF"},"wrap_text":True}
    sh.get_range_by_indexes(0,0,len(matrix),len(cols)).format.wrap_text=True
    sh.freeze_panes.freeze_rows(1)
    # bounded readable widths
    for i,c in enumerate(cols):
        width=38 if any(t in c.lower() for t in ("note","rule","treatment","implication","status","formula","purpose","complement")) else 20
        sh.get_range_by_indexes(0,i,len(matrix),1).format.column_width=width
    return sh


def build_v54_workbook(result: Dict, root: Path) -> Path:
    # Reuse the well-tested V53.1 workbook builder, then append the recovered lineage layer.
    base_path=build_v53_workbook(result,root)
    from artifact_tool import Blob, SpreadsheetFile
    wb=SpreadsheetFile.import_xlsx(Blob.load(str(base_path)))
    dash=wb.worksheets.get_item("Dashboard")
    dash.get_range("A1").values=[[V54_RELEASE_LABEL]]
    dash.get_range("A3").values=[["Version 54 restores dropped historical modules while preserving the V53.1 Earth-system headline and explicit double-count protection."]]
    rm=wb.worksheets.get_item("Read_Me")
    rm.get_range("A1:B2").values=[["Release",V54_RELEASE_LABEL],["Status",V54_RELEASE_STATUS]]
    rm.get_range("A10:B13").values=[
        ["Lineage recovery","All identified dropped modules are restored as explicit V54 modules or reconciliation screens."],
        ["Energy reconciliation","Legacy loads/supplies are not added to the 86,181.327 TWh/yr external base until that base is decomposed."],
        ["Superseded legacy conclusions","Old CO2 return dates and old zero-new-nuclear adequacy conclusions are not reinstated; current V53.1 physics/adequacy remains authoritative."],
        ["Source preservation","Legacy source versions/sheets and coupling status are listed in Lineage_Register."],
    ];rm.get_range("A10:A13").format={"font":{"bold":True}};rm.get_range("B10:B13").format.wrap_text=True
    _v54_write_sheet(wb,"Lineage_Register",result["lineage_register"],"#1F4E78")
    _v54_write_sheet(wb,"Energy_Reconcile",_v54_rows_from_dict(result["energy_reconciliation"]),"#C65911")
    r=result["restored_modules"]
    sheets=[
        ("Seaweed_SAF",r["seaweed_saf_htl"]["rows"]),("Grid_Harvester",_v54_rows_from_dict(r["grid_harvester"]["central"])),
        ("CSP_HTL",_v54_rows_from_dict(r["csp_htl"]["central"])),("Seaweed_Food",r["seaweed_food"]["rows"]),
        ("Wastewater_Energy",r["wastewater_food_waste_bioenergy"]["rows"]),("Nutrient_Loops",_v54_rows_from_dict(r["nutrient_loops_npk"]["central"])),
        ("Tidal_Energy",r["tidal_energy"]["rows"]),("Global_Maritime",r["global_maritime"]["rows"]),
        ("Storage_Portfolio",r["storage_portfolio"]["rows"]),("Firm_Clean",r["firm_clean_options"]["rows"]),
        ("Shock_Resilience",r["shock_resilience"]["rows"]),("Tree_Wealth",r["tree_wealth"]["rows"]),
        ("National_Allocation",r["national_allocation"]["rows"]),("Country_Equalizer",r["country_equalizer"]["examples"]),
        ("Parallel_Settlement",r["parallel_settlement"]["rows"]),("Sanctions_Governance",r["sanctions_governance"]["rows"]),
        ("Dollar_Transition",r["dollar_transition"]["rows"]),("No_New_Burden",r["no_new_burden"]["rows"]),
        ("Food_Health_Budget",r["food_health_budget"]["rows"]),("AI_Data_Center",r["ai_data_center_transition"]["market_rows"]),
        ("Distributed_AI",r["ai_data_center_transition"]["distributed_network_rows"]),("Cella_Archetype",r["ai_data_center_transition"]["cella_rows"]),
        ("Workforce_Transition",r["workforce_transition"]["rows"]),("Cross_Border_Fee",_v54_rows_from_dict(r["cross_border_fee"])),
        ("Resource_Screens",_v54_rows_from_dict(result["current_resource_screens"])),
    ]
    colors=["#548235","#548235","#548235","#70AD47","#70AD47","#70AD47","#4472C4","#4472C4","#8064A2","#8064A2","#C65911","#548235","#A64D79","#A64D79","#BF9000","#BF9000","#BF9000","#BF9000","#70AD47","#4472C4","#4472C4","#4472C4","#C65911","#A64D79","#548235"]
    for (name,rows),color in zip(sheets,colors): _v54_write_sheet(wb,name,rows,color)
    out=root/"Planetary_Restoration_Model_V54.xlsx";SpreadsheetFile.export_xlsx(wb).save(str(out));return out


def run_v54_tests(result: Optional[Dict]=None) -> None:
    r=result or run_v54_release(80);m=r["restored_modules"]
    assert len(m)>=22 and len(r["lineage_register"])>=23
    assert abs(m["seaweed_food"]["central"]["food_energy_kcal_person_day"]-51.8720767)<0.02
    assert abs(m["wastewater_food_waste_bioenergy"]["legacy_central"]["net_electricity_with_food_waste_twh_yr"]-865.554915)<0.01
    assert abs(m["tidal_energy"]["central"]["net_delivered_tidal_twh_yr"]-1800)<1e-9
    assert abs(m["global_maritime"]["central_total_shipping_electricity_twh_yr"]-15129.77142857143)<1e-6
    assert 15 < m["grid_harvester"]["central"]["global_harvest_electricity_twh_yr"] < 25
    assert 20000 < m["grid_harvester"]["central"]["required_fleet_units"] < 25000
    n=m["nutrient_loops_npk"]["central"];assert abs(n["effective_n_fertilizer_replacement_fraction"]-.2565)<1e-12 and abs(n["effective_k_fertilizer_replacement_fraction"]-.2775)<1e-12
    assert .45<n["n_crediting_ratio_vs_legacy_gross"]<.55 and .45<n["p_crediting_ratio_vs_legacy_gross"]<.55
    assert len(m["shock_resilience"]["rows"])==6
    assert abs(sum(m["country_equalizer"]["weights"].values())-1)<1e-12
    assert next(x for x in m["ai_data_center_transition"]["distributed_network_rows"] if x["metric"]=="Baseline Year-20 network sites")["value"]==20000
    assert next(x for x in m["ai_data_center_transition"]["cella_rows"] if x["metric"]=="Continuous IT load")["value"]==35.74
    assert r["energy_reconciliation"]["headline_adjustment_twh_yr"]==0.0
    assert r["headline"]["first_below_280_year"]==r["primary_v52_state"]["headline"]["first_below_280_year"]
    print("VERSION 54 LINEAGE-RECOVERY TESTS PASSED")


def v54_main(argv: Optional[Sequence[str]]=None) -> int:
    import argparse
    ap=argparse.ArgumentParser(description=V54_RELEASE_LABEL);ap.add_argument("--tests",action="store_true");ap.add_argument("--no-write",action="store_true");ap.add_argument("--skip-workbook",action="store_true");ap.add_argument("--ensemble-samples",type=int,default=700)
    args=ap.parse_args(argv);result=run_v54_release(args.ensemble_samples)
    _core_run_tests();_core_run_new_section_tests();run_advanced_tests(run_advanced_model());run_v52_tests(result["primary_v52_state"]);run_v53_tests(result);run_v54_tests(result)
    if not args.no_write:
        root=Path(__file__).resolve().parent;write_v54_outputs(result,root)
        if not args.skip_workbook:
            try: build_v54_workbook(result,root)
            except (ImportError,ModuleNotFoundError) as exc: print(f"WORKBOOK SKIPPED: optional artifact_tool environment unavailable: {exc}")
        build_v53_figures(result,root)
    print(json.dumps(result["headline"],indent=2,sort_keys=True));return 0
